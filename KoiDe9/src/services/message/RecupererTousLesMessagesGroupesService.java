...
public class RecupererTousLesMessagesGroupesService extends AbstractService{

	public RecupererTousLesMessagesGroupesService() {
	...
	public ResponseObject serviceLogic(RequestObject requestObject, ServletRequest servletRequest) {
		...
		if(idmsg != -1){
			...
			req.append("(SELECT uexp, idgrpdest, u.idmsg, contenu FROM listusrgroup, usertogroup u, messages m WHERE uexp = uparent AND idgrpdest = idgrp AND u.idmsg = m.idmsg AND ufils = ? ) UNION (SELECT uexp, idgrpdest, u.idmsg, contenu FROM usertogroup u, messages m WHERE u.idmsg = m.idmsg AND uexp = ?);");
		}
